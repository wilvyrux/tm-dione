			...
			if (gmMapDiv.length) {
				...
				gmMapDiv.gmap3({
					...
					marker: {
						...
						options: {
							<?php if ($marker_icon == '') { ?>
							icon: "<?php echo TM_DIONE_THEME_ROOT ?>/assets/images/map-marker.png",
							<?php } else { ?>
						...
						events: {
							click: function (marker, event) {
								...
								if (infowindow) {
									...
									infowindow.setContent("<?php echo tm_dione_esc_js( $content ); ?>");
								}
								else {
									$(this).gmap3({
										infowindow: {
											anchor: marker,
											options: {content: "<?php echo tm_dione_esc_js( $content ); ?>"}
										}
									});
								}
							}
